...
<h2><strong>Key Steps</strong></h2>
    <ol>
        <li><strong>Clean and merge datasets.</strong>
            <ol>
                <li>Run the R script to merge the census data onto the RLI data (unused for modeling but is in the Power BI report).</li>
                <li>Run the Python script to join the census dataset onto each other.</li>
                <li>Use Excel and COUNTIF functions between <strong>Merged_USCensus_Data.csv</strong> and RLI data to get postal code counts (count of RLI policies for each specific dataset). Then rename this file to <strong>Filtered_Left_Joined_Dataset</strong> and save as a CSV.</li>
            </ol>
        </li>
        <li><strong>Train machine learning models to predict target variables.</strong>
            <ol>
                <li>Run the Python script using an IDE that supports ipynb and the given dataset.</li>
            </ol>
        </li>
        <li><strong>Enhance predictions by adding geographic information</strong> (state and city) based on ZIP codes.</li>
    </ol>
